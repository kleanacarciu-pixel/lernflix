...
/** Anhang für sendMail: Dateiname und Inhalt als Bytes. */
export type MailAnhang = { filename: string; content: Buffer };
  ...
  try {
    ...
    const r = await fetch(process.env.RESEND_URL || "https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        ...(opt?.kopieAn ? { bcc: opt.kopieAn } : {}),
        // Resend erwartet den Inhalt als Base64-Zeichenkette.
        ...(opt?.anhaenge?.length
          ? { attachments: opt.anhaenge.map((a) => ({ filename: a.filename, content: a.content.toString("base64") })) }
          : {}),
      }),
    });
